...
try {
  ...
  for (const id of rewardPeriodIds) {
    ...
    await client.query({
      text: `
INSERT INTO incentives.computed_rewards (campaign_reward_period_id, locker, salt, reward_amount) ( WITH period_info AS (
		SELECT
			crp.id,
			crp.start_time,
			crp.end_time,
	...
	FROM
		...
		interval_pair_prices_without_next_start AS (
			SELECT
				date_bin (price_interval, blocks.time, '2000-01-01 00:00:00'::timestamptz) AS period_start,
				round(percentile_cont(0.5) WITHIN GROUP (ORDER BY tick_after))::int4 AS tick
			FROM
				swaps
				...
				WHERE
					block_number >= (
						...
						FROM
							...
							period_info
						WHERE
							time >= start_time - (4 * price_interval)
						ORDER BY
							number
						LIMIT 1)
				ORDER BY
