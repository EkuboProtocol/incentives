...
for (const date of dates) {
  ...
  console.log("Using pair data", date, pairData, pairDataValuesTable);
  ...
  const queryText = `
      INSERT INTO strk_defi_spring_incentives (WITH
                                                   ...
                                                   max_event_id AS (SELECT id
                                                                    ...
                                                                    WHERE block_number <= (SELECT number
                                                                                           FROM blocks
                                                                                           WHERE time < ($1::timestamptz + INTERVAL '1 day')
                                                   ...
                                                   hourly_pair_prices AS (SELECT hpp.*,
                                                                                 ...
                                                                                 INT4RANGE(CEIL(tick - (pairs.volatility_in_ticks * 0.1))::int,
                                                                                           FLOOR(tick + (pairs.volatility_in_ticks * 0.1))::int)                     AS range_1,
                                                                                 INT4RANGE(CEIL(tick - (pairs.volatility_in_ticks * 0.318639))::int,
                                                                                           FLOOR(tick + (pairs.volatility_in_ticks * 0.318639))::int)                AS range_2,
                                                                                 INT4RANGE(CEIL(tick - (pairs.volatility_in_ticks * 0.5))::int,
                                                                                           FLOOR(tick + (pairs.volatility_in_ticks * 0.5))::int)                     AS range_3,
                                                                                 INT4RANGE((tick - pairs.volatility_in_ticks)::int, (tick +
                                                                                                                                     pairs.volatility_in_ticks)::int)         AS range_4,
                                                                                 INT4RANGE((tick - pairs.volatility_in_ticks * 2)::int, (tick + pairs.volatility_in_ticks *
                                                                                                                                                2)::int)                      AS range_5,
                                                                                 INT4RANGE((tick - pairs.volatility_in_ticks * 3)::int, (tick + pairs.volatility_in_ticks *
                                                                                                                                                3)::int)                      AS range_6
